...
/*
	메세지 Null Check 하여 alert 창 호출
 */
function fncCallAlert(mssage)
{
	if (!fncIsEmpty(mssage))
	{
		alert(mssage);
		return true;
	}
...
function fncAddMssageRow(objTable, mssage)
{
	var tabRowCnt = objTable.rows.length;
	
	if (tabRowCnt > 0)
	{
		var objFirstRow = objTable.rows[0];
		var rowCellCnt = objFirstRow.cells.length;
		
		var objNewRow = objTable.insertRow(tabRowCnt);
		var objNewCell = objNewRow.insertCell(0);
		objNewCell.colSpan = rowCellCnt;
		objNewCell.innerHTML = mssage;
	}
	else
	{
